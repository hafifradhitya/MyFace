...
import { FiBookOpen, FiCamera, FiRefreshCw, FiUpload } from "react-icons/fi";
...
function Camera() {
  ...
  const isPortrait = usePotrait(); // 9:16 => 16:9
  ...
  function capturePhoto() {
    ...
    if(srcAspect > dstAspect) { 
      sh = vh; sw = Math.round(vh * dstAspect); sx = Math.round((vw - sw) / 2);
    } else {
      sw = vw; sh = Math.round(vw / dstAspect); sy = Math.round((vh - sh) / 2);
    }
  ...
  async function onPickFile(e) {
    ...
    if (f.size > 8 * 1024 * 1024) {
      setErrorMessage("Ukuran gambar terlalu besar (>8MB).");
      return;
    }
    ...
    try {
      ...
      setErrorMessage("Gagal membaca gambar. Coba lagi.");
    }
  }
  ...
  function onSubmit(e) {
    if(!photoDataUrl) {
      e.preventDefault();
      setErrorMessage("Belum ada foto, Ambil atau Pilih foto dulu")
      return;
    }
  ...
  useEffect(() => {
    ...
    const step = () => {
      if(i >= parts.length) { setIsTyping(false); return; }
      const chunk = String(parts[i++] ?? "")
      if(!chunk) { setTimeout(step, 0); return; }
    ...
    <div>
      <div className='relative w-full rounded-2xl overflow-hidden bg-black'>
        ...
        <div className="absolute bottom-2 left-1/2 -translate-1/2 flex items-center gap-3">
          ...
          {/* Add File */}
          <label
            className="flex items-center gap-2 px-3 h-14 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white cursor-pointer shadow"
          ...
          {htmlToRender.trim() ? (
            ...
            <div className='bg-gray-500 p-4'>
              <p className='font-semibold text-white'>
                Ambil Foto atau Pilih dari File, lalu tekan Ramalkan!
              </p>
            </div>
          )}
        </div>
